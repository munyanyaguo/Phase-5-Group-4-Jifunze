...
export default function Navbar() {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      console.log('[Student Navbar] Stored user:', storedUser);
      if (storedUser) {
        try {
          const userData = JSON.parse(storedUser);
          console.log('[Student Navbar] Loaded user from localStorage:', userData);
          setUser(userData);
        } catch (e) {
          console.error('[Student Navbar] Failed to parse stored user:', e);
        }
      } else {
        console.warn('[Student Navbar] No user in localStorage');
      }
      
      try {
        const freshUser = await AuthService.getCurrentUser();
        console.log('[Student Navbar] Fresh user from API:', freshUser);
        if (freshUser) {
        ...
        console.error('[Student Navbar] Failed to fetch fresh user data:', err);
      }
    };
    loadUser();
  }, []);
