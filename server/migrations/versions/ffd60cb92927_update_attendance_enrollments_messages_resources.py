...
def downgrade():
    ...
    with op.batch_alter_table("attendance") as batch_op:
        ...
        batch_op.create_unique_constraint("unique_attendance", ["user_id", "course_id", "date"])
        batch_op.create_foreign_key("attendance_user_id_fkey", "users", ["user_id"], ["id"])
        batch_op.create_foreign_key("attendance_verified_by_fkey", "users", ["verified_by"], ["id"])
        if 'user_public_id' in attendance_cols:
    ...
    enrollment_cols = [c['name'] for c in inspector.get_columns('enrollments')]
    with op.batch_alter_table("enrollments") as batch_op:
        if "user_id" not in enrollment_cols:
            batch_op.add_column(sa.Column("user_id", sa.Integer(), nullable=True))

    if 'user_public_id' in enrollment_cols:
        op.execute("""
            UPDATE enrollments e
            SET user_id = u.id
            FROM users u
    ...
    with op.batch_alter_table("enrollments") as batch_op:
        ...
        batch_op.create_foreign_key("enrollments_user_id_fkey", "users", ["user_id"], ["id"])
        if 'user_public_id' in enrollment_cols:
            batch_op.drop_column("user_public_id")
    ...
    if 'user_public_id' in messages_cols:
        op.execute("""
            UPDATE messages m
            SET user_id = u.id
            FROM users u
    ...
    resources_cols = [c['name'] for c in inspector.get_columns('resources')]
    with op.batch_alter_table("resources") as batch_op:
        if "uploaded_by" not in resources_cols:
            batch_op.add_column(sa.Column("uploaded_by", sa.Integer(), nullable=True))

    if 'uploaded_by_public_id' in resources_cols:
        op.execute("""
            UPDATE resources r
    ...
    with op.batch_alter_table("resources") as batch_op:
        batch_op.create_foreign_key("resources_uploaded_by_fkey", "users", ["uploaded_by"], ["id"])
        if 'uploaded_by_public_id' in resources_cols:
